...
namespace Quki.Bll
{
    public class UserProfileManager : BllBase<UserProfile, UserProfileViewModel>, IUserProfileService
    {
        public readonly IUserProfileRepository repo;
        public UserProfileManager(IServiceProvider service) : base(service)
        {
            repo = service.GetService<IUserProfileRepository>();
        }
        public UserProfile GetUserUserProfile(string userID)
        {
            var userProfile = TGetList(I => I.Id == userID).FirstOrDefault();
            return userProfile;
        }
        public bool AddORUpdateUserProfile(UserProfileViewModel userProfileViewModel)
        {
            bool result = false;
            var userProfile = TGetList(I => I.Id == userProfileViewModel.Id).FirstOrDefault();
            if (userProfile != null)
            {
                ...
                if (userProfileViewModel.LanguageID != null)
                    userProfile.LanguageID = userProfileViewModel.LanguageID;
